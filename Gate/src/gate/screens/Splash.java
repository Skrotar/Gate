package gate.screens;

import gate.stages.Test;
import gate.transitions.SpriteAccessor;
...
public class Splash implements Screen {
	...
	//	Gdx.graphics.setVSync(Settings.vSync());
		...
		button = new Sprite(new Texture("images/button.png"));
		...
		Tween.to(splash, SpriteAccessor.ALPHA, 1.5f).target(1).repeatYoyo(1, .5f).setCallback(new TweenCallback() {
			...
			public void onEvent(int type, BaseTween<?> source) {
				((Game) Gdx.app.getApplicationListener()).setScreen(new Test());
			}
		}).start(tweenManager);
